...
class BonsSer {
  static Future<void> Addbon(BonobjForADD bon, context) async {
    ...
    final bodyall = convert.jsonEncode({
      ...
      "IdUser": Provider.of<LocaldbProv>(context, listen: false).User,
    ...
    if (res.statusCode == 201) {
      ...
      for (var i = 0; i < Ids.length - 1; i++) {
        ...
        print("aaaaaqqqqqqqqqqqqqqq ${bon.verssement} and ${bon.Quntity}");

        for (var i = 0; i < Ids.length - 1; i++) {
          print(Ids[i]);
          print(bon.oldverss[i]);
          print(bon.verssement[i]);
        }
